...
const menu = [
  { name: "Products", icon: "i-lucide-box", path: "/dashboard/products" },
  ...
  { name: "Sales", icon: "i-lucide-badge-dollar", path: "/dashboard/sales" },
  { name: "Reports", icon: "i-lucide-bar-chart-3", path: "/dashboard/reports" },
...
<template>
  <aside class="w-full shrink-0 border-b border-slate-100 bg-white xl:min-h-screen xl:w-84 xl:border-b-0 xl:border-r">
    <!-- Brand -->
    ...
    <!-- Menu -->
    <nav class="flex gap-2 overflow-x-auto px-4 pb-4 sm:grid sm:grid-cols-2 sm:overflow-visible xl:flex xl:flex-1 xl:grid-cols-1 xl:flex-col xl:space-y-1.5 xl:px-6">
      <NuxtLink
        v-for="item in menu"
        :key="item.name"
        :to="item.path"
        class="group flex shrink-0 items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-all duration-200 sm:min-w-0 sm:gap-3 sm:px-4 sm:py-3.5 sm:text-[15px] md:gap-4 md:px-5 md:text-base xl:gap-5 xl:px-6 xl:py-4.5 xl:text-[1.1rem]"
        :class="route.path === item.path || route.path.startsWith(item.path)
          ? 'bg-slate-100/80 text-slate-950 shadow-sm shadow-slate-200/50'
          : 'text-slate-500 hover:bg-slate-50/80 hover:text-slate-800'"
      >
        <UIcon
          :name="item.icon"
          class="h-5 w-5 shrink-0 transition-colors sm:h-6 sm:w-6"
          :class="route.path === item.path || route.path.startsWith(item.path)
            ? 'text-slate-900'
    ...
    <div class="border-t border-slate-50 px-4 py-4 sm:px-6 sm:py-5 xl:mt-auto xl:px-8 xl:py-8">
      <div class="flex items-center gap-3 sm:gap-4 xl:gap-5">
        <div class="h-12 w-12 shrink-0 overflow-hidden rounded-full border-2 border-slate-100 bg-slate-50 ring-4 ring-slate-50/50 sm:h-14 sm:w-14 xl:h-15 xl:w-15">
          <img
            src="/alex_rivera.png"
            alt="Alex Rivera"
